...
- Android TV: Kotlin, fullscreen WebView shell for Quran visual runtime, AndroidX WebKit, JavaScript bridge receiver, heartbeat watchdog, recovery UI, and Media3/ExoPlayer MP4/HLS playback.
...
10. Android plays video and HLS items through a fullscreen native Media3 `PlayerView`.
11. Android sends playback completion/failure commands back to the web runtime.
12. Android watchdog reloads WebView if heartbeat stalls.
13. Cached schedule and fallback local assets prevent black screens during network failure.
...
- Current media/admin APIs:
  ...
  - `POST /api/media/scan`
- Android TV project:
  - Gradle settings: `android-tv/settings.gradle.kts`
  - Root build file: `android-tv/build.gradle.kts`
  - App build file: `android-tv/app/build.gradle.kts`
  - Manifest: `android-tv/app/src/main/AndroidManifest.xml`
  - Main fullscreen shell: `android-tv/app/src/main/kotlin/com/quran24/tv/MainActivity.kt`
  ...
  - Bridge/watchdog/Media3 runtime currently lives in `MainActivity`.
...
quran24/
  ...
  android-tv/
    app/
      src/main/
        AndroidManifest.xml
        kotlin/com/quran24/tv/
          MainActivity.kt
          HiddenSettingsActivity.kt
...
- WebView audio can be fragile for 24/7 use; native video/HLS ownership moved to Android in Phase 13, while Quran audio remains supervised through the page renderer path until later resilience phases.
